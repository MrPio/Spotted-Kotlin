...
class HomePostsAdapter(var posts: MutableList<Post?>, var loaded: Int = 0) : Adapter<ViewHolder>() {
	...
	override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder =
		if (viewType == VIEW_TYPE_POST) SpotPostViewHolder(parent.context.inflate(R.layout.spot_post))
		else object : ViewHolder(parent.context.inflate<VSpaceBinding>(R.layout.v_space).root) {}

	override fun onBindViewHolder(holder: ViewHolder, position: Int) {
		if (holder is SpotPostViewHolder) holder.bind(posts[position]!!)
	}
